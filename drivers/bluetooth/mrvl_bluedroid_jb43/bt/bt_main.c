 ...
 */
void
bt_dump_firmware_info(bt_private * priv)
{
	...
	ITCM_Ptr = vmalloc(ITCM_SIZE + 1);
	if (!ITCM_Ptr) {
		PRINTM(ERROR, "%s: Vmalloc (%d bytes) failed!\n", __func__,
		       ITCM_SIZE + 1);
		ret = BT_STATUS_FAILURE;
		goto done;
	}
	...
	if (priv->card_type == CARD_TYPE_SD8787)
		...
		dtcm_size = DTCM_SIZE;
	PRINTM(MSG, "DTCM_SIZE=0x%x\n", dtcm_size);
	DTCM_Ptr = vmalloc(dtcm_size + 1);
	if (!DTCM_Ptr) {
		PRINTM(ERROR, "%s: Vmalloc (%d bytes) failed!\n", __func__,
		       dtcm_size + 1);
		ret = BT_STATUS_FAILURE;
	...
	SQRAM_Ptr = vmalloc(SQRAM_SIZE + 1);
	if (!SQRAM_Ptr) {
		PRINTM(ERROR, "%s: Vmalloc (%d bytes) failed!\n", __func__,
		       SQRAM_SIZE + 1);
		ret = BT_STATUS_FAILURE;
	...
	PRINTM(MSG, "Start ITCM output , please wait...\n");
	...
	do {
		...
		*dbg_ptr =
		...
		if (dbg_ptr < end_ptr)
			...
			PRINTM(ERROR, "pre-allocced buf is not enough\n");
			goto done;
		}
		for (reg = reg_start; reg <= reg_end; reg++) {
			*dbg_ptr =
			...
			if (dbg_ptr < end_ptr)
				...
				PRINTM(MSG, "pre-allocced buf is not enough\n");
		}
		switch (ctrl_data) {
		case DEBUG_ITCM_DONE:
#ifdef MBT_64BIT
			PRINTM(MSG, "ITCM done: size=0x%lx\n",
			       dbg_ptr - ITCM_Ptr);
#else
			PRINTM(MSG, "ITCM done: size=0x%x\n",
			       dbg_ptr - ITCM_Ptr);
#endif
			dbg_ptr = DTCM_Ptr;
			end_ptr = DTCM_Ptr + DTCM_SIZE;
			PRINTM(MSG, "Start DTCM output, please wait...\n");
			break;
		case DEBUG_DTCM_DONE:
#ifdef MBT_64BIT
			PRINTM(MSG, "DTCM done: size=0x%lx\n",
			       dbg_ptr - DTCM_Ptr);
#else
			PRINTM(MSG, "DTCM done: size=0x%x\n",
			       dbg_ptr - DTCM_Ptr);
#endif
			dbg_ptr = SQRAM_Ptr;
			end_ptr = SQRAM_Ptr + SQRAM_SIZE;
			PRINTM(MSG, "Start SQRAM output, please wait...\n");
			break;
		case DEBUG_SQRAM_DONE:
#ifdef MBT_64BIT
			PRINTM(MSG, "SQRAM done: size=0x%lx\n",
			       dbg_ptr - SQRAM_Ptr);
#else
			PRINTM(MSG, "SQRAM done: size=0x%x\n",
			       dbg_ptr - SQRAM_Ptr);
#endif
			PRINTM(MSG, "End output!\n");
			break;
		default:
			break;
		}
	...
	if (IS_ERR(pfileITCM)) {
		PRINTM(MSG,
		       "Now Start to save all the outputs to the /var directory, please wait...\n");
		...
		PRINTM(MSG,
		       "Now Start to save all the outputs to the /data directory, please wait...\n");
		pfileDTCM =
	...
	if (IS_ERR(pfileITCM)) {
		PRINTM(MSG, "Create ITCM file failed\n");
		goto done;
	}
	if (IS_ERR(pfileDTCM)) {
		PRINTM(MSG, "Create DTCM file failed\n");
		filp_close(pfileITCM, NULL);
		goto done;
	}
	if (IS_ERR(pfileSQRAM)) {
		PRINTM(MSG, "Create SQRAM file failed\n");
		filp_close(pfileITCM, NULL);
		filp_close(pfileDTCM, NULL);
		goto done;
	}
	PRINTM(MSG, "Create ITCM/DTCM/SQRAM files OK\n");
	fs = get_fs();
	set_fs(KERNEL_DS);
	pos = 0;
